...
def convert_fsdp_checkpoints_to_hfmodels():
    ...
    # Patch chat_template if missing
    tokenizer_config_path = os.path.join(hf_path, "tokenizer_config.json")
    orig_tokenizer_config_path = os.path.join(args.hf_model_path, "tokenizer_config.json")
    ...
    if "chat_template" not in config_data:
        if os.path.exists(orig_tokenizer_config_path):
            with open(orig_tokenizer_config_path, "r") as f:
                orig_config_data = json.load(f)
            if "chat_template" in orig_config_data:
                config_data["chat_template"] = orig_config_data["chat_template"]
                with open(tokenizer_config_path, "w") as f:
                    json.dump(config_data, f, indent=2)
                print("✅ chat_template copied from original model")
            else:
                print("⚠️ Warning: chat_template not found in original tokenizer_config.json")
        else:
            print("⚠️ Warning: original tokenizer_config.json not found")
    else:
        print("✅ chat_template already present in tokenizer_config.json")
