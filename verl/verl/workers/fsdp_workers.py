...
class RewardModelWorker(Worker):
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL) ## 每个进程都进行创建
    def init_model(self):
        # This is used to import external_lib into the huggingface systems
        import_external_libs(self.config.model.get("external_lib", None))
...
class CustomRewardWorker(Worker):
    """
    Distributed-ready Reward Manager that runs on multiple GPUs via NCCL.
    """
    def __init__(
        self,
        tokenizer,
        ...
        **reward_kwargs
        ...
        # paths for BERTScorer
        # self._bert_path="../llms/bert_series/bert-base-uncased"
        # self._baseline_path="../llms/bert_series/bert-base-uncased.tsv"
        self._bert_path="/home/work/austin/MedCCO/llms/bert_series/bert-base-uncased"
        self._baseline_path="/home/work/austin/MedCCO/llms/bert_series/bert-base-uncased.tsv"
    
    @register(dispatch_mode=Dispatch.ONE_TO_ALL) 
    def init_model(self):
        # This is used to import external_lib into the huggingface systems
        self.bert_model = BERTScorer(
            model_type=self._bert_path,
            lang='en',
        ...
        self.compute_score  = batch_compute_score
        self.num_examine = 1
    ...
    def compute_rm_score(self, data: DataProto):
        ...
        #====================== Compute scores =========================
        # print("using device for cal bert score",self.bert_model.device)
        decoded = []
        ...
        scores, reward_extra_info =  self.compute_score(
        ...
        if int(torch.cuda.current_device())==0:
            ...
            for i in range(len(gts)):
                if count < self.num_examine:
                    ...
                    response_str = decoded[i]
                    ground_truth = gts[i]
                    answer_type = extras[i]['answer_type']
                    print(
                    f"""![prompt] {prompt_str}\n
                    ![response] {response_str}\n
                    ![ground_truth] {ground_truth}\n
                    ![score]: answer_type {answer_type} total: {rewards[i]:.4f}, 
                    open_bert: {reward_extra_info['open_reward_bert_score'][i]:.4f}, 
                    open_bleu: {reward_extra_info['open_reward_bleu1'][i]:.4f}, 
                    open_rouge: {reward_extra_info['open_reward_rouge1'][i]:.4f}
                    report_generation: {reward_extra_info['report_reward'][i]:.4f}
                    [score_close_format]: close_reward: {reward_extra_info['close_reward'][i]:.4f}, format_reward: {reward_extra_info['format_reward'][i]:.4f}"""
                    )
                    count += 1
        # data.non_tensor_batch['reward_extra_info'] = {}
        # for key, value in reward_extra_info.items():
        #     data.non_tensor_batch['reward_extra_info'][key] = value
